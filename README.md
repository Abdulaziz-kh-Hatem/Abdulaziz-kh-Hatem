<div align="center">

# Abdulaziz K. A. Hatem, B.Eng.
**Biomedical Engineer | Biosignal Processing, Assistive HCI & Embedded Machine Learning**

Department of Biomedical Engineering · Faculty of Engineering & Computing  
**University of Science and Technology (UST), Aden, Yemen**

[![Google Scholar](https://img.shields.io/badge/Google_Scholar-Citations-4285F4?logo=googlescholar&logoColor=white)](https://scholar.google.com/citations?user=XIzvxOUAAAAJ)
[![ORCID](https://img.shields.io/badge/ORCID-0009--0007--9681--5165-A6CE39?logo=orcid&logoColor=white)](https://orcid.org/0009-0007-9681-5165)
[![Scopus](https://img.shields.io/badge/Scopus-Author_Indexed-FF6C37?logo=elsevier&logoColor=white)](https://doi.org/10.46904/eea.26.74.2.1108016)
[![ResearchGate](https://img.shields.io/badge/ResearchGate-Profile-00CCBB?logo=researchgate&logoColor=white)](https://www.researchgate.net/profile/Abdulaziz-Hatem-3)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-abdulazizhatem-0077B5?logo=linkedin&logoColor=white)](https://linkedin.com/in/abdulazizhatem)
[![Email](https://img.shields.io/badge/Email-a.kh.hatem%40gmail.com-D14836?logo=gmail&logoColor=white)](mailto:a.kh.hatem@gmail.com)

<br/>

[![Academic Rank](https://img.shields.io/badge/Class_Rank-Top_7.5%25_(3rd%2F40)-blue?style=flat-square&logo=mortarboard)](https://github.com/Abdulaziz-kh-Hatem)
[![GPA](https://img.shields.io/badge/Cumulative_GPA-4.0%20%2F%204.0_(92.0%25)-darkgreen?style=flat-square)](https://github.com/Abdulaziz-kh-Hatem)
[![Capstone Thesis](https://img.shields.io/badge/Capstone_Grade-100%25_(Highest_Distinction)-gold?style=flat-square)](https://github.com/Abdulaziz-kh-Hatem/graduation-project)
[![Journal Publication](https://img.shields.io/badge/Publication-1st_Author_EEA_2026-teal?style=flat-square)](https://doi.org/10.46904/eea.26.74.2.1108016)
[![Hackathon Award](https://img.shields.io/badge/Hackathon-1st_Place_Winner_2026-purple?style=flat-square)](https://github.com/Abdulaziz-kh-Hatem/nabdh-vital-signs-prototype)

</div>

---

## 📌 Academic Profile

I am a Biomedical Engineering graduate from the University of Science and Technology (UST), Aden, graduating with a GPA of 4.0/4.0 and ranking 3rd among 40 students (Top 7.5%).

My main interests are biomedical instrumentation, physiological signal processing, machine learning, and artificial intelligence in healthcare. Through my academic projects, I have worked on biopotential signal acquisition, analog circuits, signal processing, embedded systems, and assistive human-computer interfaces.

### Research Interests
- Physiological Signal Processing (ECG, EOG, PPG)
- Biomedical Instrumentation and Low-Cost Medical Devices
- Machine Learning and Deep Learning for Biomedical Applications
- Artificial Intelligence in Healthcare
- Assistive Human-Computer Interfaces (HCI) and Brain-Computer Interfaces (BCI)
- Embedded Systems and TinyML for Biomedical Applications

### Academic Goal
I am interested in pursuing M.Sc. and Ph.D. studies in Biomedical Engineering, Neural Engineering, and Intelligent Health Systems, with a particular interest in physiological signals, biomedical devices, machine learning, and AI-assisted healthcare.

---

## 🎓 Academic Credentials & Quick Metrics

| Metric | Detail | Institutional Verification |
| :--- | :--- | :--- |
| **Degree** | **B.Eng. in Biomedical Engineering** (Graduation: July 2026) | UST Aden, Faculty of Engineering & Computing |
| **Academic Standing** | **Cumulative GPA: 4.0 / 4.0 (92.0%)** · **Class Rank: 3rd / 40 (Top 7.5%)** | 4-Year Dean’s List for Academic Excellence |
| **Capstone Thesis** | *Design and Implementation of an Integrated EOG-Based HCI for Virtual Keyboard & Wheelchair Control* | **Graded 100% (Distinction with Highest Honors)** |
| **Peer-Reviewed Paper** | 1 First-Author Journal Article in *Electrotehnica, Electronica, Automatica (EEA)* | **Scopus-Indexed** · DOI: [10.46904/eea.26.74.2.1108016](https://doi.org/10.46904/eea.26.74.2.1108016) |
| **National Awards** | **1st Place Winner** — Digital Fabrication & Local Solutions Hackathon (Feb 2026) | Portable Dual-Channel Vital Signs Monitor ("Nabdh") |

---

## 📄 Peer-Reviewed Publications

* **Abdulaziz K.A. Hatem** *(First Author)*, Ahmed M.A.S. AlKadhi, Mohammed A.A. Qasem, Khaled A.M. Farhan, and Nasr Kaid Ali AL-Audi.  
  **"Design and Development of a Low-Cost Electronic Platform for Electrooculography Signals Acquisition."**  
  *Electrotehnica, Electronica, Automatica (EEA)*, 2026, vol. 74, no. 2, pp. 130–137. ISSN: 1582-5175.  
  [![Scopus](https://img.shields.io/badge/Indexing-Scopus-orange.svg)](https://doi.org/10.46904/eea.26.74.2.1108016)
  [![DOI](https://img.shields.io/badge/DOI-10.46904%2Feea.26.74.2.1108016-blue.svg)](https://doi.org/10.46904/eea.26.74.2.1108016)
  [![Open Hardware Code](https://img.shields.io/badge/Hardware_Repo-eog--acquisition--platform-2ea44f.svg)](https://github.com/Abdulaziz-kh-Hatem/eog-acquisition-platform)
  [![Live Demo Video](https://img.shields.io/badge/YouTube-Oscilloscope_Validation-red.svg)](https://youtu.be/ZO9QT6c9rzA)

---

## 🔬 Featured Research & Engineering Repositories

Each project combines custom electronic hardware, signal processing, and performance evaluation:

### 1. [graduation-project](https://github.com/Abdulaziz-kh-Hatem/graduation-project)
> **Undergraduate Graduation Project | Graded 100% (Distinction with Highest Honors) | Team Lead**  
> *Integrated Electrooculography (EOG) Platform for Motorized Wheelchair Navigation & Arabic Virtual Keyboard*

* **Architecture:** 4-stage discrete Analog Front-End (AD620 instrumentation amp, active TL072 0.8–30 Hz bandpass filter, 50 Hz twin-T notch filter, gain adjustable 600×–6000×), Arduino ATmega328P 10-bit ADC, real-time MATLAB DSP pipeline, HC-05 Bluetooth telemetry, and 3D-printed chassis with ultrasonic safety override.
* **Clinical / Benchmark Performance:** Evaluated with N = 5 participants over 100 continuous trials:
  * **94.0%** directional control accuracy
  * **16.0 characters/minute** sustained typing speed on custom Arabic hierarchical virtual keyboard
* **Artifacts & Demos:** [Repository](https://github.com/Abdulaziz-kh-Hatem/graduation-project) · [Typing Demo (YouTube)](https://youtu.be/_7P_xF_lJTU) · [Wheelchair Hardware Demo (YouTube)](https://youtube.com/shorts/MNgpAQf11o8?si=srUkkPCUByLsF-Y1)

---

### 2. [ecg-pvc-detection-esp32](https://github.com/Abdulaziz-kh-Hatem/ecg-pvc-detection-esp32)
> **Biosignal Machine Learning & Embedded Systems | Undergraduate Research Project**  
> *Subject-Specific Premature Ventricular Contraction (PVC) Detection via Dual-Voting Feature Consensus & Decision Trees*

* **Architecture:** Evaluated on 36,701 beats from 15 MIT-BIH Arrhythmia Database records (Lead MLII). 3rd-order Butterworth bandpass filter (0.5–40 Hz), 300 ms beat extraction, and patient-specific 50-beat median template baseline calibration. Extracted 32 features (RR-intervals, morphology, Hjorth complexity/mobility, template correlation). Dual-voting consensus (Random Forest Gini importance + XGBoost gain) selects the top 12 features.
* **Benchmark Performance:**
  * **99.69%** mean accuracy | **98.19%** mean sensitivity | **99.82%** mean specificity (chronological held-out test splits)
  * Memory footprint **< 3 KB** — enables high-accuracy real-time arrhythmia telemetry on low-cost microcontrollers (ESP32 / ARM Cortex-M)
  * Fully verified with 35/35 automated unit & integration tests
* **Artifacts & Code:** [Repository](https://github.com/Abdulaziz-kh-Hatem/ecg-pvc-detection-esp32) · [MIT-BIH PhysioNet Dataset](https://physionet.org/content/mitdb/1.0.0/)

---

### 3. [eog-acquisition-platform](https://github.com/Abdulaziz-kh-Hatem/eog-acquisition-platform)
> **Peer-Reviewed Hardware Platform | Published in EEA Journal (2026)**  
> *Low-Cost Analog Front-End Electronic Platform for Electrooculography*

* **Architecture:** Complete analog conditioning circuit built around the AD620 instrumentation amplifier ($R_G = 100\,\Omega$, gain $495\times$), TL072 active bandpass (1.6–16 Hz), active low-pass (16 Hz cutoff), and level-shifting stage for 0–5V ADC dynamic range. Galvanically isolated via dual 9V battery supply.
* **Validation:** Oscilloscope bench testing verifying complete rejection of 50 Hz powerline interference; real-time MATLAB acquisition at 250 Hz.
* **Artifacts & Demos:** [Repository](https://github.com/Abdulaziz-kh-Hatem/eog-acquisition-platform) · [Paper DOI (10.46904/eea.26.74.2.1108016)](https://doi.org/10.46904/eea.26.74.2.1108016) · [Live Oscilloscope Demonstration (YouTube)](https://youtu.be/ZO9QT6c9rzA)

---

### 4. [ecg-acquisition-platform](https://github.com/Abdulaziz-kh-Hatem/ecg-acquisition-platform)
> **Biomedical Instrumentation & Circuit Design**  
> *Discrete Low-Cost ECG Analog Front-End (AFE) for Physiological Telemetry*

* **Architecture:** Discrete biopotential amplifier using an AD620 for high CMRR, coupled with active cascaded TL072 / LM741 operational amplifier stages to remove motion artifacts and high-frequency EMG noise.
* **Validation:** Validated on human subjects using a Hantek digital oscilloscope, capturing clear P-wave, QRS-complex, and T-wave morphologies.
* **Artifacts & Demos:** [Repository](https://github.com/Abdulaziz-kh-Hatem/ecg-acquisition-platform) · [Live Oscilloscope ECG Trace (YouTube)](https://youtu.be/GUkwhOJBexI)

---

### 5. [nabdh-vital-signs-prototype](https://github.com/Abdulaziz-kh-Hatem/nabdh-vital-signs-prototype)
> **Rapid Prototyping & Hackathon Winner | 1st Place Trophy (UST Aden, Feb 2026)**  
> *Dual-Channel ECG & PPG Physiological Acquisition Device Fabricated in 36 Hours*

* **Architecture:** Simultaneous biopotential and optical sensing using an AD8232 ECG AFE and MAX30102 pulse oximetry module (I2C), microcontroller firmware with digital signal filtering, custom 3D-printed handheld chassis, and real-time LCD metrics display.
* **Artifacts & Demos:** [Repository](https://github.com/Abdulaziz-kh-Hatem/nabdh-vital-signs-prototype) · [Live Device Testing (YouTube)](https://youtu.be/_p6Nn98H_nc)

---

## 🛠️ Core Technical Competencies

```
Biomedical Engineering & Signal Processing
├── Biosignal Conditioning : Analog Front-End (AFE) Design, Biopotential Amps (AD620, AD8232, INA), Active Multi-Stage Filters
├── Digital Signal Proc.   : Filter Design (Butterworth, Chebyshev, Notch), Baseline Drift Removal, QRS Detection, Hjorth Parameters
├── Embedded Systems & IoT : C/C++, Arduino (ATmega328P), ESP32, Sensor Interfacing (I2C, SPI, UART, ADC), Bluetooth (HC-05)
├── Machine Learning & ML  : Decision Trees, Random Forests, XGBoost, Model Compression (<3 KB for MCUs), Feature Selection
└── Scientific Software    : Python (NumPy, SciPy, Scikit-learn, WFDB, Pandas), MATLAB (DSP Toolbox), KiCad, Proteus, LaTeX, Git
```

| Domain | Tools & Technologies |
| :--- | :--- |
| **Biosignals & DSP** | Filter Design (Butterworth, Notch), Baseline Wander Suppression, Peak/Feature Extraction, Waveform Shape Analysis, Moving-Average Windowing |
| **Machine Learning & AI** | Decision Trees, Random Forest, XGBoost, Scikit-learn, Feature Importance Ranking, Patient-Specific Template Calibration, Model Quantization |
| **Analog Hardware & PCB** | AD620, AD8232, TL072, LM741, Breadboard Prototyping, PCB Schematic & Layout (KiCad, EasyEDA), Circuit Simulation (Proteus) |
| **Instrumentation & Lab** | Digital Storage Oscilloscopes (Hantek DSO5072P), Function Generators, Multimeters, Soldering, Electrical Safety & Isolation |
| **Embedded & Computing** | C/C++ (Arduino IDE), Python 3.x, MATLAB (DSP System Toolbox), Git, GitHub, LaTeX, OriginLab, Mendeley, Zotero |

---

## 🏆 Honors, Awards & Academic Distinctions

* 🥇 **1st Place Winner** — *Digital Fabrication & Local Solutions Hackathon*, UST Aden (*Feb 2026*)  
  Awarded for designing and building "Nabdh", a portable dual-channel ECG/PPG monitor within 36 hours.
* 🌟 **Youth Innovation & Entrepreneurship Award** — *Entrepreneurship Summit* (*2025*)  
  Recognized for "BioSkill Lab", a 3D medical equipment training simulator for biomedical engineering students.
* 📜 **Dean’s Honor List for Academic Excellence** — *Faculty of Engineering & Computing, UST Aden* (*2022 – 2026*)  
  Maintained top-tier academic standing (3rd rank out of 40 students, Top 7.5%) throughout all four undergraduate years.
* 🔬 **Selected Prototype Exhibitor** — *Annual Scientific Creativity & Innovation Forum* (*2023 – 2024*)  
  Demonstrated working biomedical prototypes to academic faculty, clinical engineers, and healthcare representatives.

---

## 📜 Professional Certifications

* **Supervised Machine Learning: Regression and Classification** — *Stanford Online & DeepLearning.AI* (*Mar 2026*)
* **McKinsey Forward Program** — *McKinsey & Company* (*Dec 2025*)  
  *Executive problem solving, digital agility, structured communication, and leadership under uncertainty.*
* **Understanding Research Methods** — *University of London* (*Jul 2026*)
* **Python Programming Track** — *DataCamp* (*2026*)

---

## 📊 GitHub & Research Activity

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Abdulaziz-kh-Hatem&show_icons=true&theme=algolia&hide_border=true&count_private=true" alt="GitHub Stats" height="150" />
  &nbsp;&nbsp;
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Abdulaziz-kh-Hatem&layout=compact&theme=algolia&hide_border=true" alt="Top Languages" height="150" />
</div>

---

## 🤝 Academic References & Contact

<table width="100%">
  <tr>
    <td width="50%" valign="top">
      <strong>Dr. Nasr Kaid Ali AL-Audi</strong><br/>
      Head, Department of Biomedical Engineering<br/>
      Faculty of Engineering & Computing<br/>
      University of Science and Technology, Aden<br/>
      📧 <code>n.alaudi@ust.edu</code><br/>
      <em>Role: Capstone Thesis Advisor & Publication Co-Author</em>
    </td>
    <td width="50%" valign="top">
      <strong>Dr. Mustafa Ayesh Al-Dhaheri</strong><br/>
      Department of Biomedical Engineering<br/>
      Faculty of Engineering & Computing<br/>
      University of Science and Technology, Aden<br/>
      📧 <code>m.ayesh@ust.edu</code><br/>
      <em>Role: Academic Lecturer & Department Referee</em>
    </td>
  </tr>
</table>

<div align="center">

### Direct Inquiries & Postgraduate Research Inquiries
📬 **Personal:** [a.kh.hatem@gmail.com](mailto:a.kh.hatem@gmail.com) &nbsp;|&nbsp; 🏛️ **Academic:** [a.hatem@student.ust.edu](mailto:a.hatem@student.ust.edu) &nbsp;|&nbsp; 🌐 [LinkedIn Profile](https://linkedin.com/in/abdulazizhatem)

</div>
